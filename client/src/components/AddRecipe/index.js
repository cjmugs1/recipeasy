...
const [form] = Form.useForm();
    // adds recipe
const [addRecipe, {data, error}] = useMutation(ADD_RECIPE);

const [ingredientsFormData, setIngredientsFormData] = useState({name: [], quantity: [], unit: []})

const [recipeFormData, setRecipeFormData] = useState({ name: '', description: '', ingredients: ingredientsFormData, instructions: '', tags: [], imageURL: '',  });
...
const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const newRecipe = await addRecipe({
        variables: {
                ...
                userId: userToken.data._id,
                name: "So delicious! Yummmy!",
                description: "parmesan",
                ingredients: [
                  {"name":"carrots", "quantity": "3", "unit": "carrrots"}, 
                  {"name":"greated parmesan", "quantity": "1", "unit": "cup"}],
                instructions: "chop it",
                cookingTime: {"amount": "45", "unit": "min"},
    ...
    setRecipeFormData({ name: '', description: '', ingredients: [], instructions: '', tags: [], imageURL: '',  });
    form.resetFields()
};
...
return (
    <>
      ...
      >
        ...
        <Form.Item required={true} label= "Recipe Name" name="name" onChange={handleInputChange}>
            <Input />
        </Form.Item>

        <Form.Item required={true} label ="Description" name="description" onChange={handleInputChange}>
            <Input />
        </Form.Item>
          ...
          <>
            {fields.map((field, index) => (
              ...
              >
                ...
                >
                  <Input placeholder="Quantity (eg: 2)" style={{ width: '60%' }} />
                </Form.Item>
                <Form.Item
                  ...
                  onChange={handleInputChange}
                >
                  <Input placeholder="Unit (eg: cups) Not required" style={{ width: '60%' }} />
                </Form.Item>
                <Form.Item
                ...
                rules={[
                    {
                      required: true,
                      whitespace: true,
                      message: "Please add an ingredient or delete this field.",
                    },
                ]}
                onChange={handleInputChange}
                >
                    <Input placeholder="Ingredient (eg: chopped walnuts)" style={{ width: '60%' }} />
                </Form.Item>
                </Input.Group>
    ...
    {/* instructions */}
      ...
      >
        ...
        <Form.Item >
          ...
          <Button type="submit" onClick={handleFormSubmit}>Add Recipe!</Button>
        </Form.Item>
      </Form>
    </>
  );
